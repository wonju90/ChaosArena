...
저장소, Prometheus/Slack 알림까지 갖췄다. 특히 **재해복구(DR)를 트래픽 페일오버로만 끝내지
않았다** — Jenkins/ArgoCD/모니터링까지 양쪽 리전에 완전히 대칭으로 복제해서, 한쪽 리전이
통째로 죽어도 반대쪽이 독립적으로 빌드·배포·관측을 계속할 수 있는 형태로 만들었다.
...
Jenkins/ArgoCD/Prometheus가 양쪽 리전에 각각 존재하는 이유는 **DR(재해복구)을 완전한 형태로
만들기 위해서**다. GSLB로 트래픽만 넘어가는 건 반쪽짜리 DR이다 — 컨트롤 플레인(배포·모니터링
도구)이 한쪽에만 있으면, 그 리전이 오래 죽었을 때 반대쪽 서비스는 멀쩡해도 새 코드를 배포할
방법이 없어지는 단일장애점(SPOF)이 남는다. 그래서 컨트롤 플레인까지 이중화해 KR2를 완전히
내려놓은 상태에서 KR1이 혼자 빌드→배포까지 끝내는 것을 실측 검증했다(`PROJECT_LOG.md` 4.34절).
...
├── requirements.txt
├── templates/             # 대시보드 화면 (게임 콘솔 / 실시간 대시보드 / 기록실 / CI-CD 미션 / 월드맵)
...
└── docs/
    ├── HIGHLIGHTS.md       # 빠르게 훑어보는 핵심 시나리오 + 트러블슈팅 하이라이트
...
- [`docs/HIGHLIGHTS.md`](docs/HIGHLIGHTS.md) — 빠르게 훑어보고 싶다면 여기부터: 핵심 시나리오 + 트러블슈팅 하이라이트
