...
### 4.39 발표 준비 라운드 — main 병합, 월드맵 리네이밍, 화면 정리
- **배경**: 사내 데모·스터디 발표를 앞두고 마지막으로 정리한 항목들. 인프라 기능 추가가 아니라
  "이미 만든 걸 잘 보여주기"에 가까운 작업들이라 한 절로 묶는다.
- **`main` 병합**: `infra/k8s-setup`이 `main`보다 126커밋 앞서 있던 것을 PR(#1)로 병합. Jenkins가
  `infra/k8s-setup` 브랜치만 감시하도록 설정돼 있어(4.14절) `main` 병합 자체는 배포/CI에 영향이
  없다 — 순수하게 "포트폴리오 기본 브랜치를 최신 상태로 맞추는" git 작업.
- **`README.md` 아키텍처 갱신**: 컨트롤 플레인 이중화(4.34절) 이전 상태로 멈춰있던 mermaid
  다이어그램을 `ARCHITECTURE.md`와 동일하게(양쪽 리전 대칭) 갱신, `HTTPS`로 잘못 표기돼 있던 걸
  실제 상태(`HTTP`, TLS 미착수)로 수정, GSLB Active/Standby 방향(KR1이 Active)도 같이 정정.
- **`GSLB 상태 지속시간` 실시간 카운터**: `/infra`의 페일오버 이력 중 가장 최근 이벤트의 `at`
  타임스탬프를 기준으로 "KR1 상태가 N분 M초째 유지 중"을 1초마다 갱신 — 백엔드 변경 없이
  프론트에서만 계산(기존 `failover-history` API가 이미 갖고 있던 데이터 재사용).
- **탭 이름 `인프라 지도` → `월드맵`**: "이지/하드/**보스전**" 등 이미 전투 테마 어휘를 쓰던 이
  게임의 결에 맞춰, 리전 상태를 관찰하는 이 탭도 게임스럽게 개명. 사이드바/페이지 제목만 바꾸는
  프론트 전용 변경.
- **리전별 배포 이름 화면 표시 구분**: 양쪽 리전의 실제 Deployment 이름이 둘 다 `chaos-demo`라
  화면에 나란히 놓였을 때 구분이 안 되는 문제 — 실제 k8s 리소스 이름을 바꾸는 대신(GitOps
  prune으로 인한 재생성/다운타임 리스크, Jenkins·RBAC·매니페스트 레포까지 다 고쳐야 하는 범위),
  `templates/infra.html`에서 화면에 표시할 때만 `-{region_id}` 접미사를 붙였다(`chaos-demo-kr1`/
  `chaos-demo-kr2`). 발표 3일 전에 라이브 클러스터 리소스명을 바꾸는 리스크를 피하면서 목적(구분)은
  달성.
- **고정 배지 `KR1 · KR2` 삭제**: `전체 인프라 흐름` 패널 제목 옆에 있던 이 배지는 GSLB 상태와
  무관한 하드코딩된 텍스트였고, 각 리전 레인의 `현재 트래픽` 배지가 이미 같은 정보를 정확하게
  보여주고 있어 순수 중복이라 제거.
- **페이지별 설명 문단 → 공통 "ⓘ" 툴팁 컴포넌트**: 월드맵(3중복 설명), CI/CD 미션(GitOps 설명),
  실시간 대시보드(Prometheus vs 로컬 KPI 차이 설명)에 각각 상시 노출되던 문단들이 화면을
  지저분하게 만든다는 피드백을 받고, `base.html`에 재사용 가능한 `.info-tip` 컴포넌트(호버는 CSS,
  클릭/탭은 JS 토글)를 추가해 전부 그 뒤로 숨겼다. 월드맵의 GSLB TTL 콜아웃만은 라이브 데모 해석에
  꼭 필요한 정보라 예외로 always-visible 유지.
- **발표 준비 문서 신설**: `docs/DEMO_GUIDE.md`(타임라인·대본·라이브 데모 커맨드·Q&A 대비)와
  `docs/PPT_CONTENT.md`(AI 슬라이드 생성 도구용 콘텐츠 개요) 추가.

### 4.40 발표 계획 철회 — 포트폴리오 문서 재구조화로 전환 + "컨트롤 플레인 이중화" 용어를 DR로 재정리 ⭐
- **배경**: 4.39절에서 사내 발표를 앞두고 `DEMO_GUIDE.md`/`PPT_CONTENT.md`까지 만들어뒀는데,
  실제로는 라이브 발표를 진행하지 않고 **문서 자체가 최종 결과물**이 되는 것으로 방침이 바뀌었다.
- **용어 재정리 — "컨트롤 플레인 이중화" → "DR(재해복구)"**: 강사님이 "컨트롤 플레인 이중화보다
  DR이 맞지 않냐"는 의견을 줬다. 처음엔 21절의 "서비스 경로 vs 컨트롤 플레인" 구분(사용자가
  못 느끼면 DR이 아니다)을 근거로 반대 의견을 냈지만, 지금 상태(KR1/KR2 양쪽에 서비스 경로 +
  컨트롤 플레인이 전부 대칭 설치되어 완전한 Active/Standby, 4.34절)를 다시 짚어보니 강사님
  의견이 맞았다. **정석적인 DR의 정의는 트래픽 페일오버뿐 아니라 재해 사이트에서 운영(빌드·
  배포·모니터링) 자체가 지속되는지까지 포함**하기 때문에, 컨트롤 플레인 이중화는 DR과 별개
  개념이 아니라 **그 DR을 "완전하게" 만든 마지막 조각**이었던 셈이다. "DR"을 상위 개념으로
  걸고, "컨트롤 플레인 이중화"는 "많은 DR이 트래픽 페일오버만 하고 끝나는데 우리는 CI/CD·
  모니터링까지 포함해서 재해 사이트에서 실제로 운영까지 가능한 걸 실측 검증했다"는 뒷받침
  디테일로 README/`ARCHITECTURE.md`/`CONCEPTS.md` 전체에 반영했다.
- **문서 재구조화**: 발표를 전제로 쓰인 문서(`DEMO_GUIDE.md`의 타임라인·대본·리허설 체크리스트·
  Q&A 대비표, `CONCEPTS.md`의 "🎤 발표 한 줄" 마커 31개와 말미의 발표 흐름 부록)는 옆에서
  설명을 들으며 보완하는 걸 전제하기 때문에, 혼자 읽는 포트폴리오 독자에게는 안 맞거나 어색하게
  읽힌다. 그래서 `PPT_CONTENT.md`는 삭제(그 안의 "트러블슈팅 하이라이트 3선"만 이관)하고,
  `DEMO_GUIDE.md`는 `HIGHLIGHTS.md`로 개명해 포트폴리오 워크스루 + 트러블슈팅 큐레이션
  문서로 전환했다. `CONCEPTS.md`의 발표 부록은 삭제하고 "🎤 발표 한 줄"은 "핵심 요약"으로,
  `ARCHITECTURE.md`의 "핵심 데모"·구어체 내레이션·아티팩트 참조는 셀프 리딩에 맞게 정리했다.
- **알아둘 점**: "발표를 위해 만든 자료"와 "포트폴리오로 남길 자료"는 겉보기엔 같은 내용이라도
  톤과 구조가 다르다 — 전자는 말하는 사람이 옆에서 설명을 보완하는 것을 전제하지만, 후자는
  그 설명까지 전부 문서 안에 담아야 한다.
...
- [x] **월드맵(/infra) 탭** — 양쪽 리전 상태 + GSLB 현재 트래픽 대상을 화면으로 관찰, 라이브
  검증 완료(`docs/CONCEPTS.md` 22절, 4.35~4.38절)
- [x] **발표 준비 라운드** — main 병합, README 갱신, 탭 리네이밍(인프라 지도→월드맵), 설명 문단
  정리, GSLB 상태 지속시간 카운터 (4.39절)
- [x] requirements 버전 고정 (이미 완료돼 있던 상태 확인, 4.39절에서 재확인)
- [x] **발표 계획 철회 + 포트폴리오 문서 재구조화** ⭐ — 라이브 발표 대신 문서 자체가 최종
  결과물이 되는 것으로 전환. "컨트롤 플레인 이중화" → "DR(재해복구)"로 용어를 재정리해
  README/`ARCHITECTURE.md`/`CONCEPTS.md`에 반영, `PPT_CONTENT.md` 삭제,
  `DEMO_GUIDE.md` → `docs/HIGHLIGHTS.md`로 개명 + 포트폴리오 워크스루로 재작성 (4.40절)
